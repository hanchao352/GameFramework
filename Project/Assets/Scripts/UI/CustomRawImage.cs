...
/// <summary>
/// 自定义RawImage - 继承自RawImage，添加交互事件和置灰控制
/// </summary>
[AddComponentMenu("UI/Custom/Custom RawImage")]
    ...
    IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    [Header("UV动画")]
    [SerializeField] private bool enableUVAnimation = false;
    [SerializeField] private Vector2 uvAnimationSpeed = new Vector2(0.1f, 0.1f);
    ...
    #region 事件属性 - 便捷访问（RawImage没有原生点击事件）
    /// <summary>
    /// 点击事件（RawImage原生没有）
    /// </summary>
    ...
    public bool EnableUVAnimation
    {
        get => enableUVAnimation;
        set => enableUVAnimation = value;
    }

    public Vector2 UVAnimationSpeed
    {
        get => uvAnimationSpeed;
        set => uvAnimationSpeed = value;
    }
    #endregion
    ...
    {
        Rect rect = uvRect;
        rect.x += uvAnimationSpeed.x * Time.deltaTime;
        rect.y += uvAnimationSpeed.y * Time.deltaTime;
    ...
    /// 播放视频纹理
    /// </summary>
    public void PlayVideo(UnityEngine.Video.VideoPlayer videoPlayer)
    {
        if (videoPlayer != null)
        {
    ...
    public void SetWebCamTexture(WebCamTexture webCamTexture)
    {
        ...
        // 根据相机方向调整UV
        if (webCamTexture != null && webCamTexture.videoVerticallyMirrored)
        {
    ...
    /// 创建并播放WebCam
    /// </summary>
    public WebCamTexture CreateAndPlayWebCam(int deviceIndex = 0, int width = 640, int height = 480, int fps = 30)
    {
        WebCamDevice[] devices = WebCamTexture.devices;
        if (devices.Length > deviceIndex)
        {
            WebCamTexture webCamTexture = new WebCamTexture(devices[deviceIndex].name, width, height, fps);
    ...
    /// 开始UV动画
    /// </summary>
    public void StartUVAnimation(Vector2 speed)
    {
        enableUVAnimation = true;
        uvAnimationSpeed = speed;
    }

    /// <summary>
    /// 停止UV动画
    /// </summary>
    public void StopUVAnimation(bool resetUV = false)
    {
        enableUVAnimation = false;
        if (resetUV)
        {
            uvRect = originalUVRect;
        }
    }
    #endregion
}
